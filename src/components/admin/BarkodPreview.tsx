...
// Code 128 barkodu oluşturmak için bir kütüphane eklemeniz gerekecek.
// Örneğin: npm install jsbarcode react-jsbarcode
...
interface BarkodPreviewProps {
  config: {
    ...
    // BarkodAyarlari.tsx'den eklenen yeni alanlar
    width: number; 
    height: number;
    logoUrl: string;
    footerText: string;
...
export default function BarkodPreview({ config }: BarkodPreviewProps) {
  ...
  const containerStyle: React.CSSProperties = {
    ...
    overflow: 'hidden',
    margin: '0 auto', // Sayfa ortala
    pageBreakInside: 'avoid', // Yazdırma için sayfa kırılmasını engelle
    breakInside: 'avoid', // Modern tarayıcılar için sayfa kırılmasını engelle
  };
  ...
  // Logo bölümü için düzenlenmiş stil, konumlandırma düzeltildi
  const getLogoContainerStyle = (): React.CSSProperties => {
    let logoAlign = 'left';
    
    // logoPosition değerine göre hizalama
    ...
    return {
      ...
      marginBottom: '15px',
      minHeight: '50px', // Logo alanı için daha fazla yer
      width: '50%' // Barkod ile logo arasındaki alanı dengelemek için
    };
  };
  
  // Logo için düzenlenmiş stil, boyut arttırıldı
  const logoImageStyle: React.CSSProperties = {
    maxWidth: '200px', // Logo genişliği arttırıldı
    maxHeight: '250px', // Logo yüksekliği arttırıldı
    objectFit: 'contain'
  };
    ...
    <div style={containerStyle} className="barkod-preview">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        {config.showLogo && config.logoPosition !== 'none' && config.logoUrl ? (
          ...
          // Logo gösterilmeyecekse veya URL yoksa başka bir div ile yer tutucu
